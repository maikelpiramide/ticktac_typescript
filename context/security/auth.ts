...
const isAdminOrUser = async (req:Request,res:Response,next:NextFunction)=>{
    try{
        
         const auth = req.body.auth
        if(auth.rol == Rol.ADMIN || auth.rol == Rol.USER){
...
export { decode,createTokenUser,createTokenClient,isAuth, isAdmin,isUser,isClient,isAdminOrUser}